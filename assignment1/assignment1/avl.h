...
typedef enum BalFactor { LH, EH, RH } BalFactor;
...
template<class ItemType>
void TreeType<ItemType>::PrintTree(TreeNode<ItemType>* tree) //Inorder 
{
	if (tree != NULL)
	{
		PrintTree(tree->left);
		...
		{
			cout << "RIGHT:" << tree->right->info << ":LEFT:" << tree->left->info;
		}
		cout << endl;
